...
| 2026-05-23 | **Cache-busting skill installed atop existing `bust.sh` (motherboard-v1).** Installer added: `public/cb-shapes/00..63.svg` (64 procedural shape tiles), `public/cb-badge.js` (corner-widget renderer), `scripts/fingerprint-urls.py`, and patched `index.html` head with `<meta name="cb">` + `<link rel="icon" type="image/svg+xml" href="/cb-shapes/NN.svg">`. On every bust the token (random 32-bit hex) drives URL `?v=...` fingerprinting, the cb meta tag, and the favicon cell (`token[0] mod 64`). Existing build-id stamp + `public/build-id.json` writers were preserved by appending them to the new `bust.sh`. | The two layers compose cleanly. The favicon visibly changes shape and colour across builds (different cell index, different SVG content) — the "did the new build actually ship" question is now answerable at a glance. WebP rasterization was skipped (no libcairo on kainode); the skill auto-falls back to SVG cells with identical behaviour. | [[dev]], [[ux]] |
...
## Session Log
2026-05-23 — MOTHERBOARD-V1 — installed the cache-busting skill (`bash $SKILL/scripts/install.sh --target . --webp`). 64 SVG shape cells landed in `public/cb-shapes/`; the corner-widget JS lives at `public/cb-badge.js`; `index.html` head gained `<meta name="cb">`, the shape favicon link, and the badge `<script>`. The skill's `bust.sh` overwrote the project's; we appended the existing build-id stamp + `public/build-id.json` writers to the new script (build-id is unchanged from v1, plus a `CB_TOKEN` field for cross-referencing). Verified the favicon cell + token change across consecutive `bash scripts/bust.sh` runs. `npm run build` runs the composed bust as prebuild; both layers update on every build.
